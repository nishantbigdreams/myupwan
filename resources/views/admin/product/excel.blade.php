...
@section('page-content')
    ...
    <div class="row">
                ...
                <form class="" action="index.html" method="post" id="filter_listing">
                    ...
                    @foreach ($categories as $key => $category)
                        <div class="checkbox checkbox-primary checkbox-single m-r-15 m-l-5">
                            <input id="{{$category->id}}" type="checkbox" name="category" value="{{$category->name}}">
        ...
        <div class="col-md-6">
            <div class="col-md-6">
                <a class="btn btn-custom waves-effect waves-light btn-sm" href="/admin/download_template">Download Template</a>
            </div>
            <div class="col-md-6">
                <div class="row">
                <form action="{{route('admin.excel.upload_template')}}" enctype="multipart/form-data" method="POST">
                  {{csrf_field()}}
        ...
        <div class="col-md-6" style="margin-top:5px;">
            <div class="col-md-6">
                <a class="btn btn-custom waves-effect waves-light btn-sm" href="/admin/download_product_template">Download Product Template</a>
            </div>
            <div class="col-md-6">
                <form action="{{route('admin.excel.upload_product_template')}}" enctype="multipart/form-data" method="POST">
                  {{csrf_field()}}
                  <div class="col-sm-6" style="padding-left: 0;"><input type="file" name="target_product_excel" placeholder="Upload File" required></div>
                  <div class="col-sm-6" style="padding-left: 0;"><button type="submit" class="btn btn-primary">Upload & Edit Excel</button></div>
                </form>
            </div>
        </div>
        <div class="col-md-6" style="margin-top:5px;margin-bottom: 10px;">
            <div class="col-md-6">
                <a class="btn btn-custom waves-effect waves-light btn-sm" href="/admin/download_flipcart_template">Download Flipcart Template</a>
            </div>
        </div>
        <div class="col-md-9">
...
@push('page-script')
    ...
    let archive_list = $('#archive_listing').DataTable({
        ...
        'mark':true,
        "ajax": {
            "url" : "{{url('admin/datatable/archive/listing')}}",
            "type" : "POST",
            "data" : function(d){
                d.data = $('#filter_listing').serialize()
            }
...
$(document).on('click', '.undo_archive', function(){
    ...
    swal({
        ...
        $.ajax({
            url: '{{url("admin/unarchive/product")}}',
            method: 'POST',
            data: {product:id},
        }).done(function(data, textStatus, jqXHR) {
            if (data == 'success') {
