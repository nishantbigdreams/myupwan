...
<div class="row">
    ...
    <div class="col-xs-6">
        <a href="{{route('admin.category.create')}}" class="btn btn-link w-lg">
...
<div class="form-group">
    ...
    <div class="col-sm-9">
        ...
        @if(@$categories[21]->name == @$product->category)
        ...
        <select class="js-example-basic-multiple" name="category[]" multiple="multiple" id="template" required>
            ...
            @foreach ($categories as $key => $category)
                ...
                <option value="{{$category->name}}"    data-last_prd_sku="{{$category->last_product()->sku ?? '0'}}" data-sku="{{$category->sku_initial}}" data-fields='{{$category->data}}' selected="{{$selected}}">
                    {{str_replace("%26","&",$category->name)}}
                </option>
