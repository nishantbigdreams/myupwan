...
class UserController extends Controller
{
    ...
    public function home()
    {
        ...
        $orders = Order::withoutGlobalScope('paid_orders')->where('user_id', auth()->user()->id)->get()->toArray();
...
//$code_sent='http://smsservice.fourbrothers.co.in/http-api.php?username=novasell&password=Novasell@123&senderid=NOVASL&route=1&number=9730450492&message=hello there'; 
    ...
    public function postContact(Request $request)
    {
        ...
        \Notification::route('mail', 'info.farmtoresto@gmail.com')
    ...
    public function sendLoginOtp_oldmain(Request $request)
    {
        $code = rand(1000, 9999);
        $status = Otp::create([
            'code' => $code,
        ...
        if ($status) {
            ...
            $data = array('username' => 'farmtoresto', 'password' => 'Farm@123', 'senderid' => 'FARMTR', 'route' => 1, 'number' => $request->phone, 'message' => $message);
            // Send the POST request with cURL
            $ch = curl_init('http://smsservice.fourbrothers.co.in/http-api.php');
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
    ...
    public function captureOrder(Request $request)
    {
        ...
        if (!$request->retry_payment) {
            ...
            //sms
            $msg = 'Dear ' . $order->user->name . '.Your order ' . $order->order_id . ' is successfully placed, Thank you for shopping. Team FARMTORESTO';

            $data = array('username' => 'farmtoresto', 'password' => 'Farm@123', 'senderid' => 'FARMTR', 'route' => 1, 'number' => $order->user->phone, 'message' => $msg);
            $ch = curl_init('http://smsservice.fourbrothers.co.in/http-api.php');
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            ...
            //sms to admin
            $message = 'There is new order on farmtoresto.com of customer name ' . Auth::user()->name . ' order no. ' . $order->order_id;
            $data = array('username' => 'farmtoresto', 'password' => 'Farm@123', 'senderid' => 'FARMTR', 'route' => 1, 'number' => 7715916266, 'message' => $message);
            $ch = curl_init('http://smsservice.fourbrothers.co.in/http-api.php');
            curl_setopt($ch, CURLOPT_POST, true);
            curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        ...
        if ($error->fails()) {
            ...
            if (count($u) == 0) {
                ...
                if ($user != null && \Hash::check($request->req_password, $user->password)) {
                    $data = array('username' => 'farmtoresto', 'password' => 'Farm@123', 'senderid' => 'FARMTR', 'route' => 1, 'number' => $request->phone, 'message' => 'THANK YOU FOR REGISTERING WITH US.');
                    $ch = curl_init('http://smsservice.fourbrothers.co.in/http-api.php');
                    curl_setopt($ch, CURLOPT_POST, true);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, $data);
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
